...
**Status:** draft for discussion, 28 August 2026 (revision 9: the agent-facing documentation is reshaped by ADR-0027 into a machine surface plus a contract page, which rewrites N6 and R48. Revision 8: the documentation platform is decided in ADR-0026 — Astro Starlight, in one site with the demo — which adds section 7.12 and resolves the last of section 13's site questions. Revision 7: decisions hived off into ADRs under `docs/adr/`; section 6 is now an index and a D-number map)
**Owner:** houfu
